...
1 Bursa Teknik Üniversitesi, DBMM Fakültesi, BURSA
...
Makine Öğrenmesi, Biyomedikal Veri, Weka
...
Makine Öğrenmesi (MÖ), önceki gözlemlere dayanarak doğru tahminler yapabilmeyi öğrenebilmek amacıyla otomatik tekniklerin geliştirilmesidir  [1].    Ayodele'ye  göre  ise  MÖ, otomatik  olarak  öğrenme  işlemini  deneyimlerden yola  çıkarak  geliştiren  ve  gerçekleştiren  bilgisayar sistemlerinin geliştirilmesidir [2].
...
Denetimli öğrenme modelinde problem, sınıflandırma problemi olarak ele alınır ve eğitilmiş sistem test setine yönelik tahmin ve tanıma amacıyla  kullanılır  [4].  Destek  Vektör  Makinesi, Yapay Sinir Ağları, Lojistik Regresyon, Basit Bayes, Multinom Basit Bayes, k-En Yakın Komşu, Rastgele  Orman  ve  Karar  Ağaçları  algoritmaları denetimli  öğrenme  modeli  oluşturulurken  yaygın olarak kullanılan yöntemlerdendir [8].
...
WEKA üzerinde makine öğrenmesi ve istatistik ile ilgili  pek  çok  kütüphane  hazır  olarak  gelmektedir. Örneğin veri ön işlemesi (data preprocessing), ilkelleme (regression), sınıflandırma (classification), gruplandırma (clustering), özellik  seçimi  veya  özellik  çıkarımı (feature extraction) bunlardan bazılarıdır. Ayrıca bu işlemler sonucunda çıkan neticelerinde görsel olarak gösterilmesini sağlayan görüntüleme (visualization) araçları bulunmaktadır [ 12].
...
SMO , herhangi bir ekstra matris depolama olmadan ve tüm sayısal QP (Quatratic Programming) optimizasyon adımları kullanmadan SVM QP sorununu hızlı bir şekilde çözer [18]. Bu uygulama global olarak bütün kayıp değerleri yenisiyle değiştirir ve nominal  öznitelikleri ikili olanlara dönüştürür.  Ayrıca  bütün  öznitelikleri  (attributes) önceden tanımlanmış değerlerle (default) normalize eder.
...
|               | Doğruluk-Accuracy (%)   | Doğruluk-Accuracy (%)   | Doğruluk-Accuracy (%)   | Doğruluk-Accuracy (%)   | Doğruluk-Accuracy (%)   | Doğruluk-Accuracy (%)   |
|---------------|-------------------------|-------------------------|-------------------------|-------------------------|-------------------------|-------------------------|
...
|               | Hata-Error (%)   | Hata-Error (%)   | Hata-Error (%)   | Hata-Error (%)   | Hata-Error (%)   | Hata-Error (%)   |
|---------------|------------------|------------------|------------------|------------------|------------------|------------------|
|               | NB               | RO               | SMO              | IBk              | DT               | J48              |
...
Çizelge 4 Çalışma Sonuçları - P-R-F-Kappa Metirkleri

|     | Dermatology Veri Seti için   | Dermatology Veri Seti için   | Dermatology Veri Seti için   | Dermatology Veri Seti için   |
|-----|------------------------------|------------------------------|------------------------------|------------------------------|
|     | P                            | R                            | F                            | Kappa                        |
...
- [15]  K arakoyun,  M.  ,  Hacıbeyoğlu,  M.  ,  2005. Biyomedikal Veri Kümeleri ile Makine Öğrenmesi Sınıflandırma  Algoritmalarının  İstatistiksel  olarak Karşılaştırılması, Dokuz Eylül Üniversitesi Mühendislik Fakültesi Dergisi, 16(48), (pp. 30-41).
